...
% Modified by Pranav Mamidanna to allow for trusted units only mode and to add variance explained display
	...
	case 'info'
        ...
        if DECOMP.nunits > 0
            ...
            var_exp = emgvariance(t0, t1);
            var_str = sprintf('VE (Current Window): %.1f%%', var_exp * 100);
        else
            var_str = ' ';
        end
        set (SCREEN.info.variance, 'string', var_str);
...
function hlist = get_more_handles (hlist, n)
    ...
    hlist(end+1:n) = copyobj (hlist(1), repmat(get(hlist(1),'parent'),[n-length(hlist),1]));
