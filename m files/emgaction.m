...
switch lower(opt);
    ...
    case 'trusted units only';
        global EMGLAB
        if ~isfield(EMGLAB, 'trusted_units_only')
            EMGLAB.trusted_units_only = false;
        end
        EMGLAB.trusted_units_only = ~EMGLAB.trusted_units_only;
        if EMGLAB.trusted_units_only
            emgstatus('Trusted units only mode: ON');
        else
            emgstatus('Trusted units only mode: OFF');
        end;
